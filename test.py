...
class Server():
    ...
    async def listen_socket(self, listened_socket=None):
        while True:
            try:  
                data = await self._recv_message(listened_socket)
                #TODO Обработай входящий запрос с данными.
                await self.send_data(data)  # Сюда надо передать ответ после проверки.

            except SocketException as exc:
                print(exc)
                listened_socket.close()

    async def _recv_message(self, listened_socket: socket.socket):
        message = b''
    ...
    async def send_data(self, raw_data):
        for user_socket in self.users:
            try:
                ...
                await self.main_loop.sock_sendall(user_socket, data)
